 ...
 * "<pclk>,<hact>,<hss>,<hse>,<htotal>,<vact>,<vss>,<vse>,<vtotal>[,i]"
 * (where each comma may be replaced by any sequence of punctuation).
 * pclk should be 108000/n for 4 <= n <= 16 (twice this for "fake" modes).
 */
...
static struct drm_display_mode *rp1vec_parse_custom_mode(struct drm_device *dev)
{
	...
	mode->hsync_start = vals[2];
	mode->hsync_end	  = vals[3];
	mode->htotal	  = vals[4];
	mode->vdisplay	  = vals[5];
	mode->vsync_start = vals[6];
	mode->vsync_end	  = vals[7];
	mode->vtotal	  = vals[8];
	mode->type = DRM_MODE_TYPE_DRIVER | DRM_MODE_TYPE_PREFERRED;
	mode->flags =
		DRM_MODE_FLAG_NHSYNC | DRM_MODE_FLAG_NVSYNC | DRM_MODE_FLAG_CSYNC;
	if (strchr(p, 'i'))
					      ...
					      const struct drm_display_mode *mode)
{
	...
	/* Check pixel clock is in a sane range */
	if (real_clock < 6016)
		return MODE_CLOCK_LOW;
	else if (real_clock > 27000)
		return MODE_CLOCK_HIGH;

	/* Try to match against the 525-line 60Hz mode (System M) */
	if (vtotal_2fld >= 500 && vtotal_2fld < 544 && vdisplay_2fld <= 486 &&
	    mode->htotal * vtotal_2fld > 32 * real_clock &&
	    mode->htotal * vtotal_2fld < 35 * real_clock &&
	    37 * mode->hdisplay <= 2 * real_clock) /* 54us */
		return MODE_OK;

	/* All other supported TV Systems (625-, 405-, 819-line) are 50Hz */
	if (mode->htotal * vtotal_2fld > 38 * real_clock &&
	    mode->htotal * vtotal_2fld < 42 * real_clock) {
		if (vtotal_2fld >= 608 && vtotal_2fld <= 640 && vdisplay_2fld <= 576 &&
		    37 * mode->hdisplay <= 2 * real_clock) /* 54us */
			return MODE_OK;
...
static const struct drm_display_mode rp1vec_modes[6] = {
	{ /* Full size 525/60i with Rec.601 pixel rate */
		DRM_MODE("720x480i", DRM_MODE_TYPE_DRIVER, 13500,
			 ...
			 DRM_MODE_FLAG_CSYNC | DRM_MODE_FLAG_INTERLACE)
	},
 ...
 * the default tv_mode was RP1VEC_TV_MODE_MONOCHROME, and only interlaced.
 */
...
static int rp1vec_connector_get_modes(struct drm_connector *connector)
{
	...
	limit = (tvstd < RP1VEC_TV_MODE_MONOCHROME) ? 4 : ARRAY_SIZE(rp1vec_modes);
	for (i = 0; i < limit; i++) {
